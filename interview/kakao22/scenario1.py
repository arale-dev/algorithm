...
# global skill score
user_skill_score = {i: 40000 for i in range(1, 31)}
related = {i: set() for i in range(1, 31)}
already_matched = []
...
def first_two_match():
    ...
    if len(waiting_line) >= 2:
        sorted_waiting_line = sorted([[waiting["id"], waiting["from"]] for waiting in waiting_line],
                                     key=lambda item: item[1])
        for i in range(len(waiting_line) // 2):
            pairs.append([sorted_waiting_line[i][0], sorted_waiting_line[i + 1][0]])
    return pairs
...
# 매칭 방법 3
def match_diff_with_wait(turn):
    global user_skill_score
    pairs = []
    time_ratio = 0
    # time_ratio = 100 # {'status': 'finished', 'efficiency_score': '99.9254', 'accuracy_score1': '34.4444', 'accuracy_score2': '47.6758', 'score': '178.4846'}
    waiting_line = api.get_waiting_line()
    ...
    if len(waiting_line) >= 2:
        ...
        waiting_ids = [user["id"] for user in waiting_line]
        waiting_times = {user["id"]: user["from"] for user in waiting_line}
        i_j_diff = [[i, j, abs(user_skill_score[i] - user_skill_score[j]) + (
                turn * 2 - waiting_times[i] - waiting_times[j]) * time_ratio] for i in waiting_ids for j in
                    waiting_ids if i < j]
...
def winner_always_win():
    ...
    for game in game_results:
        ...
        user_skill_score[win] = origin_center + (skill_difference) / 2
        user_skill_score[lose] = origin_center - skill_difference / 2


# 점수 업데이트 방법 2
def update_with_ratio():
    global user_skill_score
    game_results = api.get_game_result()
    for game in game_results:
        win, lose, taken = game["win"], game["lose"], game["taken"]
        ...
        user_skill_score[win] = expected_win * expected_win_ratio + expected_lose * expected_lose_ratio
        user_skill_score[lose] = expected_win * expected_lose_ratio + expected_lose * expected_win_ratio
...
def update_related():
    ...
    for game in game_results:
        ...
        # # 5만큼의 실력 차이를 무시하는 경우 : {'status': 'finished', 'efficiency_score': '99.9254', 'accuracy_score1': '34.4444', 'accuracy_score2': '47.6758', 'score': '178.4846'}
        # taken += 5
        # if taken >= 40:
        #     continue
        ...
        for related_user in related[win]:
            user_skill_score[related_user] + new_win - origin_win
        for related_user in related[lose]:
            user_skill_score[related_user] + new_lose - origin_lose
